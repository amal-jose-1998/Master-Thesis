...
\autoref{sec:pgm} describes probabilistic graphical models, including \acp{BN} and \acp{DBN}.
...
Instead of assigning a single deterministic outcome to an uncertain event, probabilistic models describe uncertainty using a probability distribution over possible outcomes.  
In probability theory, each uncertain quantity is represented as a \ac{RV} \cite{pml1Book}. 
...
Two \acp{RV} are said to be independent if the value of one variable does not affect the probability of the other. Formally, two variables $X$ and $Y$ are independent 
...
As the number of variables increases, modeling the full joint distribution becomes difficult because the number of possible dependencies also grows.  
...
In temporal models such as \acp{DBN}, this involves estimating the distribution of latent states from a sequence of observations \cite{pml1Book,koller2009probabilistic}.

\par For the \ac{DBN} shown in \autoref{fig:dbn}, the latent state is not directly observed and must therefore be inferred from the observation sequence.  
Inference is required because only the observations $O_{1:T}$ are available, while the state sequence $Z_{1:T}$ remains hidden.
