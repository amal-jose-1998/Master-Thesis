...
def prediction_visualizer_process(queue: mp.Queue, maneuver_names):
    ...
    has_prediction_by_vehicle = {0: False}
    last_frame_by_vehicle = {0: None}
    ...
    def _setup_figure(new_vehicle_ids):
        nonlocal fig, axes, status, vehicle_ids, probs_by_vehicle, has_prediction_by_vehicle, last_frame_by_vehicle
        ...
        has_prediction_by_vehicle = {
            vid: has_prediction_by_vehicle.get(vid, False)
            for vid in vehicle_ids
        }
        last_frame_by_vehicle = {
            vid: last_frame_by_vehicle.get(vid, None)
            for vid in vehicle_ids
        }
    ...
    def _draw_all():
        ...
        for idx, vid in enumerate(vehicle_ids):
            ...
            _setup_axes(ax, vid)
            if len(probs) > 0 and has_prediction_by_vehicle.get(vid, False):
                max_idx = int(np.argmax(probs))
                bars_local[max_idx].set_color('red')
                frame_val = last_frame_by_vehicle.get(vid, None)
                if frame_val is None:
                    best_lines.append(f"V{vid}: {maneuver_names[max_idx]}")
                else:
                    best_lines.append(f"V{vid} F{frame_val}: {maneuver_names[max_idx]}")
    ...
    while True:
        ...
        if isinstance(msg, dict) and msg.get('reset'):
            ...
            for vid in vehicle_ids:
                ...
                has_prediction_by_vehicle[vid] = False
                last_frame_by_vehicle[vid] = None

            _draw_all()
            continue

        if isinstance(msg, dict) and 'active_vehicle_ids' in msg:
            msg_vehicle_ids = msg.get('vehicle_ids')
            if msg_vehicle_ids is not None:
                normalized_vehicle_ids = [int(v) for v in msg_vehicle_ids]
                if normalized_vehicle_ids != vehicle_ids:
                    _setup_figure(normalized_vehicle_ids)

            active_set = {int(v) for v in msg.get('active_vehicle_ids', [])}
            for vid in vehicle_ids:
        ...
        if isinstance(msg, dict) and 'probs' in msg:
            ...
            has_prediction_by_vehicle[vid] = True
            frame_val = msg.get('frame')
            if frame_val is not None:
                try:
                    last_frame_by_vehicle[vid] = int(frame_val)
                except Exception:
                    pass
            _draw_all()
