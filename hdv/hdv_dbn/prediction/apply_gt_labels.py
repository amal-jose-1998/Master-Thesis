...
EXP_DIR = r"/home/RUS_CIP/st184634/implementation/hdv/models/paper-run-lc_b_S2_A4_tied_action"
DATA_ROOT = r"/home/RUS_CIP/st184634/implementation/hdv/data/highd"
CHECKPOINT_NAME = "final.npz"
SEMANTIC_MAP = str(Path(EXP_DIR) / "semantic_map.yaml")
...
class RuleThresholds:
    """
    Thresholds for tied_action_lc_B semantic labeling.
    ...
    Action:
      ...
      a1 = braking
      a2 = acceleration
      a3 = lane_change_lateral_maneuver
    """
    # Style / interaction context
    high_interaction_front_exists: float = 0.80
    low_interaction_front_exists_max: float = 0.65

    high_interaction_front_dx_max: float = 70.0
    low_interaction_front_dx_min: float = 100.0
    ...
    # Lane-change: a3
    lane_change_lc_present: float = 0.10
    lane_change_d_left_min_max: float = 1.30
    lane_change_d_right_min_max: float = 1.30
    lane_change_vy_abs_min: float = 0.05
    lane_change_ay_abs_min: float = 0.03
    ...
    cruise_ax_pos_frac_max: float = 0.55
...
def label_one_window_z(obs_t, feature_cols, thr: RuleThresholds, A=4, debug=False):
    """
    Assign rule-based GT label for tied_action_lc_B.
    ...
    def record(extra=None):
        ...
        if values is None:
            ...
            values = {
                ...
                "d_left_lane_min": safe("d_left_lane_min"),
                "d_right_lane_min": safe("d_right_lane_min"),
    ...
    front_thw_last = _get(obs_t, idx, "front_thw_last")

    # lateral features
    vy_last = _get(obs_t, idx, "vy_last")
    ay_last = _get(obs_t, idx, "ay_last")
    ay_neg_frac = _get(obs_t, idx, "ay_neg_frac")
    ay_pos_frac = _get(obs_t, idx, "ay_pos_frac")

    # lane-change / lane-boundary features
    lc_left_present = _get(obs_t, idx, "lc_left_present")
    lc_right_present = _get(obs_t, idx, "lc_right_present")

    d_left_lane_min = _get(obs_t, idx, "d_left_lane_min")
    d_right_lane_min = _get(obs_t, idx, "d_right_lane_min")
    ...
    # ---------------------------------------------------------
    # Priority 1: a3 = lane_change_lateral_maneuver
    # ---------------------------------------------------------
    has_lc_flag = (
        (np.isfinite(lc_left_present) and lc_left_present >= thr.lane_change_lc_present)
        or (np.isfinite(lc_right_present) and lc_right_present >= thr.lane_change_lc_present)
    )

    near_lane_boundary = (
        (np.isfinite(d_left_lane_min) and d_left_lane_min <= thr.lane_change_d_left_min_max)
        or (np.isfinite(d_right_lane_min) and d_right_lane_min <= thr.lane_change_d_right_min_max)
    )

    has_lateral_motion = (
        (np.isfinite(vy_last) and abs(vy_last) >= thr.lane_change_vy_abs_min)
        or (np.isfinite(ay_last) and abs(ay_last) >= thr.lane_change_ay_abs_min)
    )

    is_lane_change = (
        has_lc_flag
        or (near_lane_boundary and has_lateral_motion)
    )

    if is_lane_change:
        a = 3
        action_reason = "lane_change"

    else:
        # -----------------------------------------------------
        # Priority 2: a1 = braking
        # -----------------------------------------------------
        is_braking = (
        ...
        if is_braking:
            ...
            if is_accel:
                a = 2
                action_reason = "acceleration"

            else:
                # ---------------------------------------------
                # Priority 4: a0 = cruising
                # ---------------------------------------------
                is_cruising = (
    ...
    for i in range(m):
        ...
        for t in range(len(out_post)):
            ...
            rows.append([t, z, s, a, sem,
                ...
                round(v.get("d_left_lane_min", np.nan), 2),
                round(v.get("d_right_lane_min", np.nan), 2),
        ...
        headers = [
            ...
            "THW", "TTC", "lc_L", "lc_R", "dL_min", "dR_min",
